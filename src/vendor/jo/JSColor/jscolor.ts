...
const jscolor = {
  ...
  detectDir() {
    ...
    const elemBase = document.getElementsByTagName('base')
    for (let i = 0; i < elemBase.length; i += 1) {
      if (elemBase[i].href) {
        base = elemBase[i].href
      }
    }

    const elemScript = document.getElementsByTagName('script')
    for (let i = 0; i < elemScript.length; i += 1) {
      if (elemScript[i].src && /(^|\/)jscolor(\.min)?\.js([?#].*)?$/i.test(elemScript[i].src)) {
        const src = new jscolor.URI(elemScript[i].src)
  ...
  bind() {
    const e: any = document.getElementsByTagName('input')
  ...
  fireEvent(el, evnt) {
    ...
    if ((<any> document).createEventObject) {
      const ev: any = (<any> document).createEventObject()
      el.fireEvent('on' + evnt, ev)
    } else if (document.createEvent) {
      const ev: any = document.createEvent('HTMLEvents')
  ...
  URI(uri) {
    ...
    this.parse = function(parseUri) {
      const m = parseUri.match(/^(([A-Za-z][0-9A-Za-z+.-]*)(:))?((\/\/)([^\/?#]*))?([^?#]*)((\?)([^#]*))?((#)(.*))?/)
    ...
    this.toAbsolute = function(base) {
      const baseUri = new jscolor.URI(base)
      const r = this
      const t = new jscolor.URI(undefined)
  ...
  color(target, prop) {
    ...
    function drawPicker(x, y) {
      ...
      p.box.onmouseup = p.box.onmouseout = () => {
        target.focus()
      }
      p.box.onmousedown = () => {
        abortBlur = true
      ...
      p.padM.onmouseup = p.padM.onmouseout = () => {
        if (holdPad) {
          holdPad = false
      ...
      p.sldM.onmouseup = p.sldM.onmouseout = () => {
        if (holdSld) {
          holdSld = false
          jscolor.fireEvent(valueElement, 'change')
        }
    ...
    function setPad(e) {
      const posM = jscolor.getMousePos(e)
      const x = posM[0] - posPad[0]
      const y = posM[1] - posPad[1]
      switch (modeID) {
        case 0:
